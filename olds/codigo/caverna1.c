...
void map_Caverna1(int hposh, int hposv, int LOOK) {
	
	int h, v, i, j;
	int mh, mv;
	
	mapa_id = 2;			//	ID do mapa atual.				//
	mapaID = 2;				//	ID do mapa atual.				//
	
	//	Carrega primeiro Layer.	//
	mapa[mapaID].BMP[0] = carregaBitmap(mapa[mapaID].BMP[0], "../bmp/caverna1-1.bmp");
	//	Carrega segundo Layer.	//
	mapa[mapaID].BMP[1] = carregaBitmap(mapa[mapaID].BMP[0], "../bmp/caverna1-2.bmp");
...
//_______________________________________________________________________________________________________________________   
	...
	//	Teleporte.	//
	mapa[mapaID].matriz[14][34].acao = 3;		 //	Teletransporte.						//
	mapa[mapaID].matriz[14][34].ID = 0;			 //	Não bloqueia a passagem.			//
	mapa[mapaID].matriz[14][34].mapaid = 1;		 //	ID Mapa destino.					//
	mapa[mapaID].matriz[14][34].hposh = 64;		 // Posição horizontal.					//
	mapa[mapaID].matriz[14][34].hposv = 5;		 // Posição vertical.					//
	mapa[mapaID].matriz[14][34].look = sul;		 // Posição que o heroi estará voltado.	//
